<script>
	...
	const { data, z, xScale, yScale, zGet, zRange, custom } =
			...
			{#each $coords as d, i}
				{#if [hovered, selected, ...highlighted].includes($data[i][idKey])}
					<circle
						...
						cy={$yScale(d.y)}
						r={d.r}
						fill={overlayFill && $data[i][idKey] == selected
							? colorSelect
							: overlayFill &
							    highlighted.includes($data[i][idKey])
							  ? colorHighlight
							  : "none"}
						stroke={$data[i][idKey] == hovered
							? colorHover
							: $data[i][idKey] == selected
							  ? colorSelect
							  : colorHighlight}
						stroke-width={$data[i][idKey] == hovered ||
						$data[i][idKey] == selected ||
						highlighted.includes($data[i][idKey])
							? lineWidth
							: 0}
